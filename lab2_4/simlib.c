...
simulation_run_schedule_event(Simulation_Run_Ptr simulation_run,
			      Event_Type new_event_type, double new_event_time,
			      void *event_attachment)
{
  Event_Ptr current_event, new_event, next_event;
  ...
  TRACE(event_print_type(new_event_type);)
  ...
  if (new_event_time < current_time) {
    ...
    current_time);
    printf("Event scheduled = \"%s\"\n", new_event_type.event_description);
    exit(1);
  }

  new_event = (Event_Ptr) xmalloc(sizeof(Event));
  new_event->occurrence_time = new_event_time;
  new_event->event_type = new_event_type;
  new_event->data_ptr = event_attachment;
  ...
  if (event_list->size == 0) {
    /* The list is empty. */
    event_list->front_ptr = new_event;
    event_list->back_ptr = new_event;
    event_list->size++;
    return event_id++;
  }
  ...
  if (event_list->front_ptr->occurrence_time > new_event_time) {
    ...
    event_list->front_ptr->previous_event = new_event;
    new_event->next_event = event_list->front_ptr;
    event_list->front_ptr = new_event;
  ...
  if (event_list->back_ptr->occurrence_time <= new_event_time) {
    ...
    event_list->back_ptr->next_event = new_event;
    new_event->previous_event = event_list->back_ptr;
    event_list->back_ptr = new_event;
  ...
  current_event = event_list->front_ptr;
  next_event = event_list->front_ptr->next_event;

  while(next_event->occurrence_time <= new_event_time) {
    current_event = next_event;
    next_event = current_event->next_event;
  }
  current_event->next_event = new_event;
  new_event->previous_event = current_event;
  next_event->previous_event = new_event;
...
{
  int i;
  Event_Ptr current_event, found_event=NULL, next_event, previous_event;
  ...
  for(i=0; i<event_list->size; i++) {
    ...
    if(current_event->event_id == event_id) {
      ...
      previous_event = found_event->previous_event;
      next_event = found_event->next_event;

      /* Front of list. Adjust the front pointer. */
      if (event_list->front_ptr == found_event)
        event_list->front_ptr = next_event;

      /* Back of list. Adjust the back pointer (could be both fron and
	 back). */
      if (event_list->back_ptr == found_event)
        event_list->back_ptr = previous_event;
      ...
      content_ptr = found_event->data_ptr;

      TRACE(printf("At %.2f : ", simulation_run_get_time(simulation_run));)
      TRACE(event_print_type(found_event->event_type);)
      TRACE(printf("descheduled\n");)

      free((void*) found_event);
      event_list->size--;
      break;
    }
...
simulation_run_get_event(Simulation_Run_Ptr simulation_run)
{
  ...
  top_event = event_list->front_ptr;
  ...
  if (event_list->size == 1) {
    ...
    top_event->next_event->previous_event = NULL;
    event_list->front_ptr = top_event->next_event;
    event_list->size--;
  }
  return (top_event);
}
...
simulation_run_execute_event(Simulation_Run_Ptr simulation_run)
{
  ...
  current_event = simulation_run_get_event(simulation_run);
  simulation_run_set_time(simulation_run, current_event->occurrence_time);

  TRACE(printf("\n");)
  TRACE(event_print_type(current_event->event_type);)
  TRACE(printf("occurring at %.3f\n", simulation_run_get_time(simulation_run));)

  (*(current_event->event_type.event_function))(simulation_run,
						current_event->data_ptr);
  xfree(current_event);
}
...
static void
event_print_type(Event_Type event)
{
  printf("%s ", event.event_description);
}
